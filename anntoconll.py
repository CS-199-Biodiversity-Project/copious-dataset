    ...
    for i, l in enumerate(lines):
        ...
        for o in range(start, end):
            if o in offset_label:
                if o != start:
                    print ('Warning: annotation-token boundary mismatch: "%s" --- "%s"' % (token, offset_label[o].text))
    ...
    for t1 in textbounds:
        for t2 in textbounds:
            ...
            if t1.end-t1.start > t2.end-t2.start:
                print ("Eliminate %s due to overlap with %s" % (t2, t1))
                eliminate[t2] = True
            else:
                print ("Eliminate %s due to overlap with %s" % (t1, t2))
                eliminate[t1] = True
